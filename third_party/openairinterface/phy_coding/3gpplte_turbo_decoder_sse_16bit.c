...
//  int16x8_t tmp128[(n+8)>>3];
  ...
  switch (crc_type) {
  case OPENAIR_CRC24_A:
    crc_len=3;
    break;
  case OPENAIR_CRC24_B:
    crc_len=3;
    break;
  ...
  log_map16(systematic0,yparity1,m11,m10,alpha,beta,ext,n,0,F,offset8_flag);
  ...
  while (iteration_cnt++ < max_iterations) {
    ...
    log_map16(systematic2,yparity2,m11,m10,alpha,beta,ext2,n,1,F,offset8_flag);
...
// Mask64 = 2^b0 + 2^b1 + 2^b2 + 2^b3 + 2^b4 + 2^b5 + 2^b6 + 2^b7
    ...
    if (iteration_cnt>1) {
      ...
      case OPENAIR_CRC24_A:
        ...
        {
          oldcrc&=0x00ffffff;
          crc = openair_crc24a(&decoded_bytes[F>>3],
                       n-24-F)>>8;
          temp=((uint8_t *)&crc)[2];
      ...
      case OPENAIR_CRC24_B:
        oldcrc&=0x00ffffff;
        crc = openair_crc24b(decoded_bytes,
                     n-24)>>8;
        temp=((uint8_t *)&crc)[2];
        ((uint8_t *)&crc)[2] = ((uint8_t *)&crc)[0];
        ((uint8_t *)&crc)[0] = temp;
      ...
      case OPENAIR_CRC16:
        oldcrc&=0x0000ffff;
        crc = openair_crc16(decoded_bytes,
                    n-16)>>16;
        break;

      case OPENAIR_CRC8:
        oldcrc&=0x000000ff;
        crc = openair_crc8(decoded_bytes,
                   n-8)>>24;
        break;
    ...
    if (iteration_cnt < max_iterations) {
      log_map16(systematic1,yparity1,m11,m10,alpha,beta,ext,n,0,F,offset8_flag);
#if defined(__x86_64__) || defined(__i386__)
