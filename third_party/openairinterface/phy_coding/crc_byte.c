...
static unsigned int             poly24a = 0x864cfb00;   //1000 0110 0100 1100 1111 1011  D^24 + D^23 + D^18 + D^17 + D^14 + D^11 + D^10 + D^7 + D^6 + D^5 + D^4 + D^3 + D + 1
static unsigned int             poly24b = 0x80006300;    // 1000 0000 0000 0000 0110 0011  D^24 + D^23 + D^6 + D^5 + D + 1
static unsigned int             poly16 = 0x10210000;    // 0001 0000 0010 0001            D^16 + D^12 + D^5 + 1
static unsigned int             poly12 = 0x80F00000;    // 1000 0000 1111                 D^12 + D^11 + D^3 + D^2 + D + 1
static unsigned int             poly8 = 0x9B000000;     // 1001 1011                      D^8  + D^7  + D^4 + D^3 + D + 1
/*********************************************************
...
*********************************************************/
unsigned int
openair_crc24a (unsigned char * inptr, int bitlen)
{
...
unsigned int
openair_crc24b (unsigned char * inptr, int bitlen)
{
...
unsigned int
openair_crc16 (unsigned char * inptr, int bitlen)
{
  int             octetlen, resbit;
  unsigned int             crc = 0;
  octetlen = bitlen / 8;        /* Change in octets */
...
unsigned int
openair_crc12 (unsigned char * inptr, int bitlen)
{
  int             octetlen, resbit;
  unsigned int             crc = 0;
  octetlen = bitlen / 8;        /* Change in octets */
...
unsigned int
openair_crc8 (unsigned char * inptr, int bitlen)
{
  int             octetlen, resbit;
  unsigned int             crc = 0;
  octetlen = bitlen / 8;        /* Change in octets */
...
int main()
{
  ...
  printf("%x\n", openair_crcbit(test, sizeof(test) - 1, poly24));
  printf("%x\n", openair_crc24(test, (sizeof(test) - 1)*8));
  printf("%x\n", openair_crcbit(test, sizeof(test) - 1, poly8));
  printf("%x\n", openair_crc8(test, (sizeof(test) - 1)*8));
