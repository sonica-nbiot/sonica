...
uint32_t openair_sub_block_interleaving_turbo(uint32_t D, uint8_t *d,uint8_t *w)
{
  ...
  for (col=0; col<32; col++) {
    ...
    for (row=0; row<RTC; row++) {
      ...
      if (w[k]== OPENAIR_LTE_NULL)
        nulled++;

      if (w[Kpi+(k<<1)] ==OPENAIR_LTE_NULL)
        nulled++;

      if (w[Kpi+1+(k<<1)] ==OPENAIR_LTE_NULL)
        nulled++;
...
uint32_t sub_block_interleaving_cc(uint32_t D, uint8_t *d,uint8_t *w)
{
  ...
  for (col=0; col<32; col++) {
    ...
    for (row=0; row<RCC; row++) {
      ...
      if (w[Kpi+k] ==OPENAIR_LTE_NULL)
        nulled++;

      if (w[(Kpi<<1)+k] ==OPENAIR_LTE_NULL)
        nulled++;
...
uint32_t openair_generate_dummy_w(uint32_t D, uint8_t *w,uint8_t F)
{
  ...
  for (col=0; col<32; col++) {
    ...
    if (index<(ND+F)) {
      w[k]   =  OPENAIR_LTE_NULL;
      wKpi[k2] = OPENAIR_LTE_NULL;
#ifdef RM_DEBUG
      nulled+=2;
#endif
    }
    ...
    if ((index+32)<(ND+F)) {
      w[k+1]   =  OPENAIR_LTE_NULL;
      wKpi2[k2] = OPENAIR_LTE_NULL;
#ifdef RM_DEBUG
      nulled+=2;
#endif
    }
    ...
    if ((index+64)<(ND+F)) {
      w[k+2]   =  OPENAIR_LTE_NULL;
      wKpi4[k2] = OPENAIR_LTE_NULL;
#ifdef RM_DEBUG
      nulled+=2;
#endif
    }
...
uint32_t generate_dummy_w_cc(uint32_t D, uint8_t *w)
{
  ...
  for (col=0; col<32; col++) {
    ...
    if (index<ND) {
      w[k]          = OPENAIR_LTE_NULL;
      w[Kpi+k]      = OPENAIR_LTE_NULL;
      w[(Kpi<<1)+k] = OPENAIR_LTE_NULL;
#ifdef RM_DEBUG_CC
      nulled+=3;
#endif
    }
    ...
    if (index+64<ND) {
      w[k+2]          = OPENAIR_LTE_NULL;
      w[Kpi+2+k]      = OPENAIR_LTE_NULL;
      w[(Kpi<<1)+2+k] = OPENAIR_LTE_NULL;
    #ifdef RM_DEBUG
      nulled+=3;
    #endif
    }
                              ...
                              uint8_t r)
{
  ...
  if (Ncb>(3*(RTC<<5)))
    fprintf(stderr, "Error: Exiting, RM condition (Ncb %d, RTC %d, Nir/C %d, Nsoft %d, Kw %d)\n",Ncb,RTC,Nir/C,Nsoft,3*(RTC<<5));
  ...
  for (; (ind<Ncb)&&(k<E); ind++) {
    ...
    if (w[ind] != OPENAIR_LTE_NULL) e2[k++]=w[ind];
  }
                            ...
                            uint32_t *E_out)
{
  ...
  for (; (ind<Ncb)&&(k<E); ind++) {
    if (dummy_w[ind] != OPENAIR_LTE_NULL) {
      w[ind] += soft_input2[k++];
#ifdef RM_DEBUG
  ...
  for (k=0;k<E;k++) {
    ...
    while(dummy_w[ind] == OPENAIR_LTE_NULL) {
  #ifdef RM_DEBUG
...
void main()
{
  ...
  // initialize 96 first positions to "OPENAIR_LTE_NULL"
  for (i=0; i<96; i++)
    d[i]=OPENAIR_LTE_NULL;

  RTC = openair_sub_block_interleaving_turbo(4+(192*8), &d[96], w);
