...
int openair_lte_segmentation(unsigned char *input_buffer,
                             ...
                             unsigned int *F)
{
  ...
  if ((*C)>OPENAIR_MAX_DLSCH_PAYLOAD_BYTES) {
  ...
  *F = ((*Cplus)*(*Kplus) + (*Cminus)*(*Kminus) - (Bprime));
  ...
  if ((input_buffer) && (output_buffers)) {
	...
	//	printf("encoding segment %d : byte %d (%d) => %d\n",r,k,Kr>>3,input_buffer[s]);
      ...
      if (*C > 1) { // add CRC
        crc = openair_crc24b(output_buffers[r],Kr-24)>>8;
...
int main()
{
  ...
  for (Bbytes=5; Bbytes<2*768; Bbytes++) {
    openair_lte_segmentation(0,0,Bbytes<<3,&C,&Cplus,&Cminus,&Kplus,&Kminus,&F);
