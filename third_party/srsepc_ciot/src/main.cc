...
int main(int argc, char* argv[])
{
  ...
  srslte::log_ref s11u_log{"S11U"};
  s11u_log->set_level(level(args.log_args.mme_gtpc_level));
  s11u_log->set_hex_limit(args.log_args.mme_gtpc_hex_limit);
  ...
  if (mme->init(&args.mme_args, &nas_log, &s1ap_log, &mme_gtpc_log, s11u_log)) {
