...
#include "srsepc_ciot/mme/mme_gtpc.h"
...
static uint8_t old_dummy_pkt[84] = {
    0x45, 0x00, 0x00, 0x54, 0x80, 0xd2, 0x40, 0x00,
    0x40, 0x01, 0xfd, 0xb4, 0xac, 0x10, 0x00, 0x02,
    0x08, 0x08, 0x08, 0x08, 0x08, 0x00, 0x8c, 0x11,
    0x00, 0x01, 0x00, 0x04, 0x90, 0xd3, 0x32, 0x61,
    0x00, 0x00, 0x00, 0x00, 0xa2, 0xb4, 0x06, 0x00,
};

static uint8_t dummy_pkt[84] = {
    0x45, 0x00, 0x00, 0x54, 0x80, 0xd2, 0x40, 0x00,
    0x40, 0x01, 0xfd, 0xb4, 0xac, 0x10, 0x00, 0x02,
    0x08, 0x08, 0x08, 0x08, 0x08, 0x00, 0x8c, 0x11,
    0x00, 0x01, 0x00, 0x04, 0x90, 0xd3, 0x32, 0x61,
...
bool mme_gtpc::init(s11u_ep *ep, srslte::log_filter* mme_gtpc_log)
{
  ...
  m_s11u_ep->set_teid_to_imsi_map(&m_mme_ctr_teid_to_imsi);
...
void mme_gtpc::handle_s11_pdu(srslte::byte_buffer_t* msg)
{
  ...
  m_mme_gtpc_log->console("MME Received GTP-C PDU. Message type %s\n", srslte::gtpc_msg_type_to_str(pdu->header.type));
...
void mme_gtpc::send_s11u_pdu(uint32_t mme_teid, srslte::byte_buffer_t* msg){
  m_s11u_ep->send_s11u_pdu(mme_teid, msg);
}

bool mme_gtpc::send_create_session_request(uint64_t imsi, bool cp_ciot)
{
  ...
  printf("NB-IoT: send_create_session_request: Map TEID %d to IMSI %ld\n",cs_req->sender_f_teid.teid, imsi);
...
void mme_gtpc::handle_modify_bearer_response(srslte::gtpc_pdu* mb_resp_pdu)
{
  ...
  // Test fucntions for IP data pkt transmission
  //  srslte::byte_buffer_t* pdu = pool->allocate();
  //  memcpy(pdu->msg, dummy_pkt, sizeof(dummy_pkt));
  //  pdu->N_bytes = sizeof(dummy_pkt);
  //  m_s11u_ep->send_s11u_pdu(1, pdu);
  //  pool->deallocate(pdu);
...
bool mme_gtpc::get_s11u_teid(uint64_t imsi, uint32_t* teid)
{
  std::map<uint64_t, struct gtpc_ctx>::iterator it = m_imsi_to_gtpc_ctx.find(imsi);
  if (it == m_imsi_to_gtpc_ctx.end()) {
    m_mme_gtpc_log->error("S11U TEID not found for IMSI %lx\n", imsi);
    return false;
  }

  *teid = it->second.mme_ctr_fteid.teid;
  return true;
}
