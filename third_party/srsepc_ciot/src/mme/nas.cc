...
#include "srsepc_ciot/mme/s1ap_nas_transport.h"
...
bool nas::handle_nb_attach_request(uint32_t                enb_ue_s1ap_id,
                                struct sctp_sndrcvinfo* enb_sri,
                                srslte::byte_buffer_t*  nas_rx,
                                const nas_init_t&       args,
                                const nas_if_t&         itf,
                                srslte::log*            nas_log)
{
  ...
  // Get NAS Attach Request and PDN connectivity request messages
  LIBLTE_ERROR_ENUM err = liblte_mme_unpack_nb_attach_request_msg((LIBLTE_BYTE_MSG_STRUCT*)nas_rx, &attach_req);
  if (err != LIBLTE_SUCCESS) {
    nas_log->error("NB Error unpacking NAS attach request. Error: %s\n", liblte_error_text[err]);
    nas_log->console("NB Error unpacking NAS attach request. Error: %s\n", liblte_error_text[err]);
    return false;
  }
  // Get PDN Connectivity Request*/
  err = liblte_mme_unpack_pdn_connectivity_request_msg(&attach_req.esm_msg, &pdn_con_req);
  if (err != LIBLTE_SUCCESS) {
    nas_log->error("NB Error unpacking NAS PDN Connectivity Request. Error: %s\n", liblte_error_text[err]);
    nas_log->console("NB Error unpacking NAS PDN Connectivity Request. Error: %s\n", liblte_error_text[err]);
    return false;
  }
  ...
  // Log Attach Request Information
  nas_log->console("NB Attach request -- eNB-UE S1AP Id: %d\n", enb_ue_s1ap_id);
  nas_log->info("NB Attach request -- eNB-UE S1AP Id: %d\n", enb_ue_s1ap_id);
  nas_log->console("NB Attach request -- Attach type: %d\n", attach_req.eps_attach_type);
  nas_log->info("NB Attach request -- Attach type: %d\n", attach_req.eps_attach_type);
  nas_log->console("NB Attach Request -- UE Network Capabilities EEA: %d%d%d%d%d%d%d%d\n",
                   attach_req.ue_network_cap.eea[0],
                   attach_req.ue_network_cap.eea[1],
                   attach_req.ue_network_cap.eea[2],
                   attach_req.ue_network_cap.eea[3],
                   ...
                   attach_req.ue_network_cap.eea[6],
                   attach_req.ue_network_cap.eea[7]);
  nas_log->info("NB Attach Request -- UE Network Capabilities EEA: %d%d%d%d%d%d%d%d\n",
                attach_req.ue_network_cap.eea[0],
                attach_req.ue_network_cap.eea[1],
                attach_req.ue_network_cap.eea[2],
                attach_req.ue_network_cap.eea[3],
                ...
                attach_req.ue_network_cap.eea[6],
                attach_req.ue_network_cap.eea[7]);
  nas_log->console("NB Attach Request -- UE Network Capabilities EIA: %d%d%d%d%d%d%d%d\n",
                   attach_req.ue_network_cap.eia[0],
                   attach_req.ue_network_cap.eia[1],
                   attach_req.ue_network_cap.eia[2],
                   attach_req.ue_network_cap.eia[3],
                   ...
                   attach_req.ue_network_cap.eia[6],
                   attach_req.ue_network_cap.eia[7]);
  nas_log->info("NB Attach Request -- UE Network Capabilities EIA: %d%d%d%d%d%d%d%d\n",
                attach_req.ue_network_cap.eia[0],
                attach_req.ue_network_cap.eia[1],
                attach_req.ue_network_cap.eia[2],
                attach_req.ue_network_cap.eia[3],
  ...
  nas_log->console("NB Attach Request -- UE Network Capabilities s1u_data=%d, upciot=%d, cpciot=%d.\n",
                   attach_req.ue_network_cap.s1udata,
                   attach_req.ue_network_cap.upciot,
                   attach_req.ue_network_cap.cpciot);
  nas_log->info("NB Attach Request -- UE Network Capabilities s1u_data=%d, upciot=%d, cpciot=%d.\n",
                   attach_req.ue_network_cap.s1udata,
                   attach_req.ue_network_cap.upciot,
                   attach_req.ue_network_cap.cpciot);
                                                ...
                                                srslte::log*                                          nas_log)
{
  ...
  nas_log->console("NB Attach Request -- handle_imsi_attach_request_unknown_ue.\n");
...
// Service Requests
bool nas::handle_nb_control_plane_service_request(uint32_t                m_tmsi,
                                 uint32_t                enb_ue_s1ap_id,
                                 struct sctp_sndrcvinfo* enb_sri,
                                 srslte::byte_buffer_t*  nas_rx,
                                 const nas_init_t&       args,
                                 ...
                                 srslte::log*            nas_log)
{
  ...
  // TODO： Add the decipher function for the esm_msg
  uint8 ul_msg_count = nas_rx->msg[5];
  nas_log->debug_hex(cp_service_req.esm_msg.msg, cp_service_req.esm_msg.N_bytes, "Encrypted");
  nas_ctx->cipher_decrypt_esm_msg(&cp_service_req.esm_msg, ul_msg_count);
  nas_log->debug_hex(cp_service_req.esm_msg.msg, cp_service_req.esm_msg.N_bytes, "Decrypted");

  uint8* esm_msg_ptr = cp_service_req.esm_msg.msg;
...
//    // Re-generate K_eNB TODO: NB-IoT Do not need it
//    srslte::security_generate_k_enb(sec_ctx->k_asme, sec_ctx->ul_nas_count, sec_ctx->k_enb);
//    nas_log->info("Generating KeNB with UL NAS COUNT: %d\n", sec_ctx->ul_nas_count);
//    nas_log->console("Generating KeNB with UL NAS COUNT: %d\n", sec_ctx->ul_nas_count);
//    nas_log->info_hex(sec_ctx->k_enb, 32, "Key eNodeB (k_enb)\n");
//    nas_log->console("UE Ctr TEID %d\n", emm_ctx->sgw_ctrl_fteid.teid);
//
//    // Stop T3413 if running TODO: NB-IoT Do not need it
//    if (mme->is_nas_timer_running(T_3413, emm_ctx->imsi)) {
//      mme->remove_nas_timer(T_3413, emm_ctx->imsi);
//    }

    // Save UE ctx to MME UE S1AP id TODO: NB-IoT May not need it, remove it later
    s1ap->add_nas_ctx_to_mme_ue_s1ap_id_map(nas_ctx);
    s1ap->send_initial_context_setup_request(imsi, 5);
    ...
    if (emm_ctx->state == EMM_STATE_REGISTERED) {
      ...
      gtpc->send_modify_bearer_request(emm_ctx->imsi, 5, &nas_ctx->m_esm_ctx[5].enb_fteid);
    }
...
void nas::cipher_decrypt_esm_msg(LIBLTE_BYTE_MSG_STRUCT* esm_msg, uint8_t count)
{
  ...
  switch (m_sec_ctx.cipher_algo) {
    ...
    case srslte::CIPHERING_ALGORITHM_ID_128_EEA1:
      srslte::security_128_eea1(&m_sec_ctx.k_nas_enc[16],
                                ...
                                esm_msg->N_bytes,
                                &tmp_pdu.msg[0]);
      memcpy(&esm_msg->msg[0], &tmp_pdu.msg[0], esm_msg->N_bytes);
      m_nas_log->debug_hex(tmp_pdu.msg, esm_msg->N_bytes, "Decrypted");
      break;
...
bool nas::handle_esm_data_transport(srslte::byte_buffer_t*  nas_rx)
{
  ...
  srslte::byte_buffer_t* pdu = pool->allocate();
  memcpy(pdu->msg, esm_msg.user_data.msg, esm_msg.user_data.N_bytes);
  pdu->N_bytes = esm_msg.user_data.N_bytes;

  uint32_t teid = 0;
  bool teid_found = m_gtpc->get_s11u_teid(m_emm_ctx.imsi, &teid);
...
bool nas::handle_authentication_response(srslte::byte_buffer_t* nas_rx)
{
  ...
  // TODO: Bypass the auth for testing. Remove in future
  ue_valid = true;
...
bool nas::pack_attach_accept(srslte::byte_buffer_t* nas_buffer)
{
  ...
  // For NB-IoT, Set the eps_network_feature_support_present to true
  attach_accept.eps_network_feature_support_present = true;

  // For NB-IoT, Set the additional_update_result_present to true
  attach_accept.additional_update_result_present    = true;
  attach_accept.additional_update_result = LIBLTE_MME_ADDITIONAL_UPDATE_RESULT_CPCIOT;
...
bool nas::send_esm_data_transport(srslte::byte_buffer_t* user_data)
{
  srslte::byte_buffer_t* nas_tx;
  nas_tx = m_pool->allocate();

  m_nas_log->info("Sending ESM Data Transport\n");

  LIBLTE_MME_NB_ESM_MSG_STRUCT esm_msg;
  esm_msg.eps_bearer_id = 5;
  esm_msg.prot_disc = LIBLTE_MME_PD_EPS_SESSION_MANAGEMENT;
  esm_msg.trans_id = 0x00;

  esm_msg.user_data.N_bytes = user_data->N_bytes;
  for (uint32_t i=0; i<user_data->N_bytes; i++){
    esm_msg.user_data.msg[i] = user_data->msg[i];
  }

  uint8_t sec_hdr_type = LIBLTE_MME_SECURITY_HDR_TYPE_INTEGRITY_AND_CIPHERED;
//  uint8_t sec_hdr_type = LIBLTE_MME_SECURITY_HDR_TYPE_PLAIN_NAS;
  m_sec_ctx.dl_nas_count++;
  LIBLTE_ERROR_ENUM err = liblte_mme_pack_esm_data_transport(
      &esm_msg, sec_hdr_type, m_sec_ctx.dl_nas_count, (LIBLTE_BYTE_MSG_STRUCT*)nas_tx);
  if (err != LIBLTE_SUCCESS) {
    m_nas_log->error("Error packing EMM Information\n");
    m_nas_log->console("Error packing EMM Information\n");
    return false;
...
bool nas::pack_service_accept(srslte::byte_buffer_t* nas_buffer, uint16_t ebi_info)
{
  ...
  service_accept.eps_bearer_context_status = ebi_info;
  service_accept.t3446_present = false;
