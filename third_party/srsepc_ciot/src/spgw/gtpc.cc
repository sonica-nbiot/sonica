                                              ...
                                              const struct srslte::gtpc_modify_bearer_request& mb_req)
{
  ...
  if (tunnel_ctx->paging_pending == true) {
    ...
    m_gtpu->send_all_queued_packets(tunnel_ctx->dw_user_fteid, tunnel_ctx->paging_queue, tunnel_ctx->is_s11u);
  }
...
spgw_tunnel_ctx_t* spgw::gtpc::create_gtpc_ctx(const struct srslte::gtpc_create_session_request& cs_req)
{
  ...
  tunnel_ctx->is_s11u = cs_req.s11u_tunnel;

  // Note that with S11-U, the IP address does not matter for now, and we share
  // TEID space with S1-U (they can be separate spaces)
