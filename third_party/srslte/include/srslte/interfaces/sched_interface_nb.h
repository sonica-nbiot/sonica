...
#define SRSLTE_SCHED_INTERFACE_NB_H
...
public:
  ...
  typedef struct {
    ...
    srslte_nbiot_dci_rar_grant_t grant;
...
//    uint16_t rnti;
  ...
  typedef struct {
    ...
    ul_sched_data_t  npusch[MAX_DATA_LIST];
...
//  virtual int ul_sr_info(uint32_t tti, uint16_t rnti)                                                          = 0;
  ...
  virtual int ul_wait_timer(uint16_t rnti, uint32_t wait_time, bool set_value = true)                        = 0;
...
//  virtual int ul_cqi_info(uint32_t tti, uint16_t rnti, uint32_t enb_cc_idx, uint32_t cqi, uint32_t ul_ch_code) = 0;
  ...
  virtual int dl_sched(uint32_t hfn, uint32_t tti, dl_sched_res_t& sched_result) = 0;
  virtual int ul_sched(uint32_t hfn, uint32_t tti, ul_sched_res_t& sched_result) = 0;
...
#endif // SRSLTE_SCHED_INTERFACE_NB_H
