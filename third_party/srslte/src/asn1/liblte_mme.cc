...
LIBLTE_ERROR_ENUM liblte_mme_pack_nb_eps_network_feature_support_ie(LIBLTE_MME_EPS_NETWORK_FEATURE_SUPPORT_STRUCT* eps_nfs,
                                                                 uint8**                                        ie_ptr)
{
  LIBLTE_ERROR_ENUM err = LIBLTE_ERROR_INVALID_INPUTS;

  if (eps_nfs != NULL && ie_ptr != NULL) {
    // NB-IoT: According to NAS 9.9.3.12
    (*ie_ptr)[0] = 2; // Length
    (*ie_ptr)[1] = 1 << 7; // CPCIOT Bit
    (*ie_ptr)[1] |= eps_nfs->esrps << 5;
    (*ie_ptr)[1] |= (eps_nfs->cs_lcs & 0x03) << 3;
    (*ie_ptr)[1] |= eps_nfs->epc_lcs << 2;
    (*ie_ptr)[1] |= eps_nfs->emc_bs << 1;
    (*ie_ptr)[1] |= eps_nfs->ims_vops;
    (*ie_ptr)[2] = 1 << 3; // ePCO Bit
    *ie_ptr += 3;
...
LIBLTE_ERROR_ENUM liblte_mme_unpack_nb_ue_network_capability_ie(uint8**                                  ie_ptr,
                                                             LIBLTE_MME_UE_NETWORK_CAPABILITY_STRUCT* ue_network_cap)
{
  LIBLTE_ERROR_ENUM err = LIBLTE_ERROR_INVALID_INPUTS;
  uint32            length;
...
//      liblte_mme_pack_eps_network_feature_support_ie(&attach_accept->eps_network_feature_support, &msg_ptr);
      liblte_mme_pack_nb_eps_network_feature_support_ie(&attach_accept->eps_network_feature_support, &msg_ptr);
    }
...
LIBLTE_ERROR_ENUM liblte_mme_unpack_nb_attach_request_msg(LIBLTE_BYTE_MSG_STRUCT*               msg,
                                                       LIBLTE_MME_ATTACH_REQUEST_MSG_STRUCT* attach_req)
{
  LIBLTE_ERROR_ENUM err     = LIBLTE_ERROR_INVALID_INPUTS;
  uint8*            msg_ptr = msg->msg;
  ...
  if (msg != NULL && attach_req != NULL) {
    ...
    // UE Network Capability
    liblte_mme_unpack_nb_ue_network_capability_ie(&msg_ptr, &attach_req->ue_network_cap);
...
LIBLTE_ERROR_ENUM liblte_mme_unpack_nb_control_plane_service_request_msg(LIBLTE_BYTE_MSG_STRUCT*                msg,
                                                                         LIBLTE_MME_CONTROL_PLANE_SERVICE_REQUEST_MSG_STRUCT* cp_service_req)
{
  ...
  if (msg != NULL && cp_service_req != NULL) {
    ...
    // Service Type & NAS Key Set Identifier
    liblte_mme_unpack_service_type_ie(&msg_ptr, 0, &cp_service_req->ctrl_plane_service_type);
    liblte_mme_unpack_nas_key_set_id_ie(&msg_ptr, 4, &cp_service_req->nas_ksi);
    msg_ptr++;
    ...
    if (LIBLTE_MME_NAS_MSG_CONTAINER_IEI == *msg_ptr) {
      msg_ptr++;
      // TODO: Currently Do not support NAS msg parsing, tmp using the esm
      liblte_mme_unpack_esm_message_container_ie(&msg_ptr, &cp_service_req->nas_msg);
    ...
    // Device Properties
    if ((LIBLTE_MME_EXTENDED_SERVICE_REQUEST_DEVICE_PROPERTIES_IEI << 4) == (*msg_ptr & 0xF0)) {
      liblte_mme_unpack_device_properties_ie(&msg_ptr, 0, &cp_service_req->device_props);
      msg_ptr++;
      cp_service_req->device_props_present = true;
    } else {
      cp_service_req->device_props_present = false;
    }
...
/*********************************************************************
    IE Name: NB-IoT ESM Message Container
    ...
    Document Reference: 24.301 v10.2.0 Section 9.9.3.15
*********************************************************************/
LIBLTE_ERROR_ENUM liblte_mme_pack_nb_esm_message_container_ie(LIBLTE_BYTE_MSG_STRUCT* esm_msg, uint8** ie_ptr)
{
  LIBLTE_ERROR_ENUM err = LIBLTE_ERROR_INVALID_INPUTS;
  uint32            i;
...
LIBLTE_ERROR_ENUM liblte_mme_pack_esm_data_transport(LIBLTE_MME_NB_ESM_MSG_STRUCT* esm_msg,
                                                     uint8                                  sec_hdr_type,
                                                     uint32                                 count,
                                                     LIBLTE_BYTE_MSG_STRUCT*                msg)
{
  ...
  if (esm_msg != NULL && msg != NULL) {
    ...
    *msg_ptr = (esm_msg->eps_bearer_id << 4) | (LIBLTE_MME_PD_EPS_SESSION_MANAGEMENT);
    msg_ptr++;

    // Procedure Transaction ID
    *msg_ptr = esm_msg->trans_id;
    msg_ptr++;

    // Message Type
    *msg_ptr = LIBLTE_MME_USER_DATA_CONTAINER_IEI;
    msg_ptr++;
    liblte_mme_pack_esm_message_container_ie(&esm_msg->user_data, &msg_ptr);
...
LIBLTE_ERROR_ENUM liblte_mme_unpack_esm_data_transport(LIBLTE_BYTE_MSG_STRUCT*       msg,
                                                       LIBLTE_MME_NB_ESM_MSG_STRUCT* esm_msg)
{
  ...
  if (esm_msg != NULL && msg != NULL) {
    ...
    // EPS Bearer ID
    esm_msg->eps_bearer_id = (*msg_ptr & 0xF0) >> 4;
    msg_ptr++;

    // Procedure Transaction ID
    esm_msg->trans_id = *msg_ptr;
    msg_ptr++;

    // Skip Message Type
    msg_ptr++;

    liblte_mme_unpack_esm_message_container_ie(&msg_ptr, &esm_msg->user_data);
...
LIBLTE_ERROR_ENUM liblte_mme_unpack_nb_esm_message_container_ie(uint8** ie_ptr, LIBLTE_MME_NB_ESM_MSG_STRUCT* esm_msg)
{
  LIBLTE_ERROR_ENUM err = LIBLTE_ERROR_INVALID_INPUTS;
  uint32            i;
  uint32_t esm_msg_len;

  esm_msg->eps_bearer_id = ((*ie_ptr)[0] >> 4);
  esm_msg->prot_disc = (*ie_ptr)[0] & 0x0F;
  *ie_ptr += 1;

  esm_msg->trans_id = (*ie_ptr)[0];
  *ie_ptr += 1;

  if ((*ie_ptr)[0] != LIBLTE_MME_USER_DATA_CONTAINER_IEI){
    printf("LIBMME: liblte_mme_unpack_nb_esm_message_container_ie: Do not contain user data! Error!\n");
    return err;
  }else{
    *ie_ptr += 1;
  }
  ...
  if (ie_ptr != NULL && esm_msg != NULL) {
    esm_msg->user_data.N_bytes = (*ie_ptr)[0] << 8;
    esm_msg->user_data.N_bytes |= (*ie_ptr)[1];

    if (esm_msg->user_data.N_bytes > LIBLTE_MAX_MSG_SIZE_BYTES) {
      return err;
    }

    for (i = 0; i < esm_msg->user_data.N_bytes; i++) {
      esm_msg->user_data.msg[i] = (*ie_ptr)[2 + i];
    }
    *ie_ptr += esm_msg->user_data.N_bytes + 2;
...
/*********************************************************************
    IE Name: NB-IoT Service Accpet

    Description: sent by the network in response to the SERVICE REQUEST message

    Document Reference: 24.301 v15.8.0 Section 8.2.34
*********************************************************************/
                                                         ...
                                                         LIBLTE_BYTE_MSG_STRUCT*               msg)
{
  ...
  if (service_accept != NULL && msg != NULL) {
    ...
    *msg_ptr = LIBLTE_MME_MSG_TYPE_SERVICE_ACCEPT;
    ...
    // PACK eps_bearer_context_status
    *msg_ptr = LIBLTE_MME_EPS_BEARER_CONTEXT_STATUS_IEI;
    msg_ptr++;
    *msg_ptr = 0x02; // eps_bearer_context_status length
    msg_ptr++;

    *msg_ptr = (service_accept->eps_bearer_context_status >> 8) & 0xFF;
    msg_ptr++;
    *msg_ptr = (service_accept->eps_bearer_context_status) & 0xFF;
    msg_ptr++;
    ...
    LIBLTE_BYTE_MSG_STRUCT*                                            msg)
{
  ...
  if (act_def_eps_bearer_context_req != NULL && msg != NULL) {
    ...
    *msg_ptr = LIBLTE_MME_MSG_TYPE_ACTIVATE_DEFAULT_EPS_BEARER_CONTEXT_REQUEST;
    ...
    if (act_def_eps_bearer_context_req->ctrl_plane_only_ind_present) {
      *msg_ptr = 0x91; // According to TS 24.301 V15.8.0 Section 9.9.4.23
      msg_ptr++;
    }
