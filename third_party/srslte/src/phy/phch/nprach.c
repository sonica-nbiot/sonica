...
#include "srslte/phy/phch/nprach.h"
#include "srslte/phy/utils/debug.h"
#include "srslte/phy/utils/vector.h"

#define FREQ_SHIFT_MAX_LEN SRSLTE_NPRACH_SAMP_SIZE * 4 * 6

static void generate_freq_shift(srslte_nprach_t *q)
{
...
int srslte_nprach_init(srslte_nprach_t *q)
{
  int ret = SRSLTE_ERROR;

  if (q != NULL) {
    bzero(q, sizeof(srslte_nprach_t));

    if (srslte_dft_plan(&q->fft, SRSLTE_NPRACH_SAMP_SIZE, SRSLTE_DFT_FORWARD, SRSLTE_DFT_COMPLEX)) {
    ...
    q->fft_in  = srslte_vec_cf_malloc(SRSLTE_NPRACH_SAMP_SIZE);
    q->fft_out = srslte_vec_cf_malloc(SRSLTE_NPRACH_SAMP_SIZE);
    q->freq_shift = srslte_vec_cf_malloc(FREQ_SHIFT_MAX_LEN);

    generate_freq_shift(q);

    q->cp_len  = SRSLTE_NPRACH_SAMP_SIZE; // TODO: Format 266.7us only
...
int srslte_nprach_free(srslte_nprach_t* q)
{
  free(q->fft_in);
  free(q->fft_out);
  free(q->freq_shift);
...
int srslte_nprach_detect_reset(srslte_nprach_t *q)
{
  q->nxt_sym  = 0;
  q->cur_symg = 0;
...
static uint32_t get_subcarrier_sg(uint32_t start_sc, uint32_t sg)
{
  ...
  case 1:
    ...
    break;
  case 2:
    sc_id = (start_sc < SRSLTE_NPRACH_SUBC_HALF ?
             start_sc + SRSLTE_NPRACH_SUBC_HALF :
             start_sc - SRSLTE_NPRACH_SUBC_HALF) ^ 1;
    break;
  case 3:
    sc_id = start_sc < SRSLTE_NPRACH_SUBC_HALF ?
            start_sc + SRSLTE_NPRACH_SUBC_HALF :
...
{
  for (uint32_t start_sc = 0; start_sc < SRSLTE_NPRACH_SUBCARRIERS; start_sc++) {
    uint32_t sc = get_subcarrier_sg(start_sc, sg);
    buf_entry[start_sc] = fft_output[sc];
  }
}

int srslte_nprach_detect(srslte_nprach_t *q,
                         cf_t*            signal,
                         uint32_t         sig_len,
                         ...
                         uint32_t*        ind_len)
{
  ...
  while (rem_input + buf_offset >= SRSLTE_NPRACH_SAMP_SIZE) {
    ...
    if (q->nxt_sym == 0) {
      ...
      read_amount = SRSLTE_NPRACH_SAMP_SIZE - buf_offset;
      srslte_vec_cf_copy(&q->fft_in[buf_offset], &signal[in_offset], read_amount);

      srslte_vec_prod_ccc(q->fft_in, q->freq_shift, q->fft_in, SRSLTE_NPRACH_SAMP_SIZE);
      srslte_dft_run(&q->fft, q->fft_in, q->fft_out);

      uint32_t sym_num = q->cur_symg * SRSLTE_NPRACH_SYM_GROUP_SIZE + q->nxt_sym - 1;
      uint32_t subc_offset = (SRSLTE_NPRACH_SAMP_SIZE - 48) / 2 + q->base_subc;
    ...
    buf_offset = 0;
    if (q->nxt_sym == SRSLTE_NPRACH_SYM_GROUP_SIZE) {
      q->nxt_sym = 0;
      q->cur_symg++;
      if (q->cur_symg == 4) {
